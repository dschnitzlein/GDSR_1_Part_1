...
## In diesem ersten Teil arbeiten wir nur interaktiv in der R-Konsole
...
Die `plot()` Funktion ist der einfachste (und direkteste) Weg Daten in `R` zu plotten. Z.B. `plot(x, y)` ergibt einen Scatterplot der Daten in `x` und `y`. Bitte beachten Sie, die `plot()` Funktion ist vor allem in der Analysephase wichtig und hilfreich. Die Abbildungen sind in der Regel **nicht** hübsch im Sinne von **publikationsreif**. Eine große Stärke von `R` ist es jedoch auch publikationsreife Abbildungen erstellen zu können. Im Regelfall verwenden Sie hierfür aber dann das Paket `ggplot2`.

Das Erstellen von solchen Abbildungen wird später im Seminar ein eigener Schwerpunktblock sein.
...
Beachten Sie, oben haben wir `Einrückungen` genutzt, um den Code lesbarer zu machen. R erkennt (fast immer) automatisch ob ein Befehl der über mehrere Zeilen geht zusammenhängt. RStudio bietet Ihnen diese Einrückungen automatisch an. Es gibt auch Plug-Ins um diese explizit sichtbar zu machen.
...
Oft müssen wir nur einen Teil eines Datensatzes untersuchen. Nehmen wir an, unser Datensatz ist in der Matrix A gespeichert.
...
Ein `-` Zeichen im Index weist `R` an, alle Reihen oder Spalten auszuwählen, außer den angegeben.
...
Über die `dim()` Funktion erfahren wir die Anzahl der Reihen gefolgt von der Anzahl der Spalten einer Matrix.
...
Wir werden hier im Kurs dazu ebenfalls einen Schwerpunkttermin machen.
...
Bevor wir versuchen, einen Datensatz zu laden, müssen wir sicherstellen, dass `R` nach den Daten im richtigen Verzeichnis suchen kann. Die Einzelheiten der Vorgehensweise hängen jedoch vom verwendeten Betriebssystem (z. B. Windows, Mac, Linux) ab, so dass wir hier nicht weiter darauf eingehen.
...
Der folgende Befehl lädt die Datei `Auto.data` in `R` und speichert sie als Objekt namens `Auto` in einem Format, das als `DataFrame` bezeichnet wird. Sobald die Daten geladen sind, kann die Funktion `View()` verwendet werden, um sie in einem Excel-ähnlichen Fenster in RStudio anzuzeigen. Alternativ können wir mit `head()` die ersten Zeilen anzeigen lassen.
...
Dieser spezielle Datensatz wurde z.B. nicht korrekt geladen, weil `R` angenommen hat, dass die Variablennamen Teil der Daten sind und sie daher in die erste Zeile aufgenommen hat. Der Datensatz enthält auch eine Reihe fehlender Beobachtungen, die durch ein Fragezeichen `?` gekennzeichnet sind. Fehlende Werte sind in realen Datensätzen häufig anzutreffen. Die Verwendung der Option `header = T` (oder `header = TRUE`) in der Funktion `read.table()` sagt `R`, dass die erste Zeile der Datei die Variablennamen enthält, und die Verwendung der Option `na.strings` sagt `R`, dass jedes Mal, wenn es ein bestimmtes Zeichen (z.B. ein Fragezeichen) sieht, dieses Element als fehlendes Element in der Datenmatrix behandelt werden soll.
...
Das Argument `stringsAsFactors = T` teilt `R` mit, dass jede Variable, die (nicht-numerische) Zeichen enthält, als qualitative Variable interpretiert werden soll und dass jede einzelne Zeichenkombination eine Einheit für diese qualitative Variable darstellt.
